...
function buildDocumentHtml(document, company = {}) {
  ...
  const logo = settings.showLogoPdf !== false && company.logo_uri ? `<img src="${escapeHtml(company.logo_uri)}" style="max-height:70px;max-width:180px" />` : '';
  const bank = settings.showBankPdf !== false && settings.bankDetails ? `<p><strong>Coordonnées bancaires</strong><br>${escapeHtml(settings.bankDetails)}</p>` : '';
  const conditions = settings.generalConditions ? `<p><strong>Conditions générales</strong><br>${escapeHtml(settings.generalConditions)}</p>` : '';
  const footer = settings.footer ? `<footer style="border-top:1px solid #D7E5F5;margin-top:30px;padding-top:12px;font-size:11px">${escapeHtml(settings.footer)}</footer>` : '';
  return `<html><head><style>body{font-family:Arial;padding:32px;color:#1459C7}table{width:100%;border-collapse:collapse;margin-top:24px}th{background:#1459C7;color:#fff;padding:10px;font-size:11px;text-align:left}td{border-bottom:1px solid #D7E5F5;padding:11px;font-size:12px}.number{width:7%;text-align:center}.label{width:43%}.quantity{width:12%;text-align:center}.unit,.total{width:19%;text-align:right}.total{font-weight:bold} .grand-total{border-top:2px solid #1677FF;margin-top:22px;padding-top:14px;text-align:right;font-size:18px;font-weight:bold}</style></head><body>${logo}<h1>${escapeHtml(company.nom || 'QuickDevis')}</h1><p>${escapeHtml(company.adresse || '')}<br>${escapeHtml(company.telephone || '')} ${escapeHtml(company.email || '')}</p>${bank}<hr><h2>${escapeHtml(document.type)} ${escapeHtml(document.numero)}</h2><p>Client : ${escapeHtml(document.client_nom || 'Sans client')}</p><table><thead><tr><th class="number">N°</th><th>Libellé / Produit</th><th class="quantity">Nombre</th><th class="unit">Prix unitaire</th><th class="total">Total</th></tr></thead><tbody>${rows}</tbody></table><div class="grand-total">TOTAL : ${money(document.total)}</div>${conditions}${footer}</body></html>`;
}
